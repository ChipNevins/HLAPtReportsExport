...
namespace HLAPtReportsExport
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                string CS = ConfigurationManager.ConnectionStrings["HLA_Reports"].ConnectionString;
                ...
                using (SqlConnection conPk = new SqlConnection(CS))
                {
                    ...
                    foreach (DataRow dr in dsReportPks.Tables[0].Rows)
                    {
                        ...
                        reportName = BuildReport(reportPk);
                        textBox3.Text = reportName;
                        textBox2.Text = "Row " + nbrReportsWritten.ToString();
        ...
        private string BuildReport(int aReportPk)
        {
            string CS = ConfigurationManager.ConnectionStrings["HLA_Reports"].ConnectionString;
            DateTime dt;
            string fileName;
            bool isRnumFound = false;
            ...
            if (isRnumFound)
            {
                using (Document document = new Document())
                {
                    ...
                    foreach (DataRow drRep in ds.Tables["ReportHdrs"].Rows)
                    {
                        ...
                        tableRep.SetWidths(new int[] { 12, 65, 30, 40, 15, 15, 8 });
                        ...
                        foreach (DataRow drItem in ds.Tables["ReportItems"].Rows)
                        {
                            if (drRep.ItemArray[1].ToString() == drItem.ItemArray[1].ToString())
                            {
                                ...
                                if (drItem["dtReceived"].ToString() == "")
                                {
                                    tableItem.AddCell(new Phrase(" ", font5));
                                }
                                else
                                {
                                    dt = DateTime.Parse(drItem["dtReceived"].ToString());
                                    tableItem.AddCell(new Phrase(dt.ToShortDateString(), font5));
                                }
                ...
                return "Report for " + fileName + " complete.";
            }
            else
            {
                return "No R number found.";
            }
        }
